...
#-------------------------------------------------------------------------# 	
        if op.type == 17:
                ...
                cb = Message()
                cb.to = op.param1
                cb.text = random.choice(KAC).getContact(op.param2).displayName + ", สมาชิกใหม่ สวัสดีจร้า \n\nยินดีต้อนรับ ,  " + random.choice(KAC).getContact(op.param2).displayName + " , เข้าสู่ กลุ่ม " + group.name 
...
#-----------------------------------------------------------------------------#
                    ...
                    for i in gid:
                        cl.rejectGroupInvitation(i)
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"All Group Invites Canceled")
                    else:
                        cl.sendText(msg.to,"Done")
#----------------------------- TAG ALL MEMBER -------------------------------#
